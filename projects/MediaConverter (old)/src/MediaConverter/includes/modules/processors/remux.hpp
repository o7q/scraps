...
void module_remux()
{
    ...
    cout << "\n SELECT A FORMAT OR ENTER YOUR OWN\n"
         << "  VIDEO\n"
         << dye::bright_white(draw_array(OBJECT_STORAGE::DATA::COMMON_MEDIA_FORMATS, 1, 6, "   [#] ", true))
         << "  AUDIO\n"
         << dye::bright_white(draw_array(OBJECT_STORAGE::DATA::COMMON_MEDIA_FORMATS, 7, 13, "   [#] ", true))
         << "  IMAGE\n"
         << dye::bright_white(draw_array(OBJECT_STORAGE::DATA::COMMON_MEDIA_FORMATS, 14, 17, "   [#] ", true));
    draw_cursor();

    string formatSelect;
    getline(cin, formatSelect); syncCin();

    string format = isInt(formatSelect) ? '.' + OBJECT_STORAGE::DATA::COMMON_MEDIA_FORMATS[stoi(formatSelect) - 1] : formatSelect.front() == '.' ? formatSelect : '.' + formatSelect;
