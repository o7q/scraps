...
A program that calculates the total number of unique combinations on a 4x4 grid containing 6 pieces in-which they are layed out in a way where there is an even amount of them per row and column. (challenge from my math teacher, note that the final number may change if you are unlucky, the higher `MAX_ATTEMPTS` is, the more accurate the answer will be.) \
